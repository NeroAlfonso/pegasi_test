...
api.get('/', (req, res) => res.setHeader('content-type', 'text/plain').send('System Technologies Api v1'))

const server =api.listen(
        (process.env.NODE_ENV !== 'test' ? process.env.API_PORT : 0),
        async () => console.log(`System technologies Api iniciada en el puerto ${(process.env.API_PORT)}`)
);
